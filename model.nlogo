...
to move-towards-the-stairs [person]
    ask person [
     let target-patch min-one-of (patches with [patch-type = "stairs"]) [distance myself]
...
to move-along-corridor [person]
  ...
  ask person [
    ...
    let x [pxcor] of one-of patches with [patch-type = "platform" and number = num ]
    ifelse x > xcor [
       set heading 90
       forward 1
    ][
...
to set-up-station
  ask patches [
    build-platform self 4 (max-pxcor - platform-size) max-pxcor
